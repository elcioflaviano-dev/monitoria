...
with CONTEUDO_TV.container():
    ...
    # -------------------------------------------------------------------------
        ...
        if os.path.exists(ARQUIVO_ROTA_DISCO):Élcio, erro de digitação meu! Peço desculpas. 

Na linha `797` (na Tela 3 dos Indicadores), eu digitei `(c for d in ...)` em vez de `(c for c in ...)`. O Python não encontrou a letra "c" e acusou o `NameError`.

Já corrigi essa letrinha. Copie o código abaixo, selecione tudo (Ctrl+A) no seu arquivo **`Painel_Rotativo.py`**, apague e cole a versão definitiva:
