...
# ... (Mantenha aqui as suas LISTAS FIXAS e a função obter_nome_visual) ...
...
# TEMPOS DAS TELAS
esperas = {0: 60, 1: 30 if alerta_fim_janela else 60, 2: 30 if alerta_fim_janela else 180, 4: 2}

tempo_passado = time.time() - st.session_state.last_time

if tempo_passado > esperas.get(st.session_state.idx, 30):
    if antes_0830:
        st.session_state.idx = 0 if st.session_state.idx != 0 else 0
    else:
        # CICLO: 0 -> 4 -> 1 -> 4 -> 2 -> 4 -> 0
