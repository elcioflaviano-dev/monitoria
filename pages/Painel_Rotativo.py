...
<style>
    ...
    .block-container { padding-top: 1rem !important; padding-bottom: 75px !important; max-width: 98% !important; }
    ...
    /* BOTAO PROXIMA OCULTO/FANTASMA NO CANTO INFERIOR DIREITO */
    div[data-testid="stButton"] { position: fixed !important; bottom: 75px !important; right: 20px !important; z-index: 999999 !important; display: flex !important; justify-content: flex-end !important; width: auto !important; }
    div[data-testid="stButton"] > button { background-color: #003366 !important; color: #ffffff !important; border: 2px solid #ffffff !important; border-radius: 30px !important; padding: 8px 20px !important; font-size: 18px !important; font-weight: bold !important; opacity: 0.03 !important; transition: all 0.4s ease-in-out !important; }
    ...
    /* ESTILOS DA CAIXA BASE - GRANDES E BONITOS */
    .box-base { 
        background: #e8f5e9; 
    ...
    .num-base { font-size: 100px !important; font-weight: 900; color: #111; line-height: 1; }
    
    /* CARDS DOS SUPERVISORES PARA O TEC1 */
    .box-contagem { 
        background: #ffffff; 
    ...
    /* CARDS SUPERVISORES PARA AS DEMAIS TELAS (ROTA E CONSULTIVO) */
    .sup-card { 
        background: #ffffff; 
        border: 2px solid #e0e0e0; 
        border-radius: 12px; 
    ...
    .badge-faltas { background: #ffebee; color: #c62828; padding: 6px 15px; border-radius: 10px; font-size: 20px !important; font-weight: 900; border: 3px solid #ffcdd2; }
    ...
    .ticker-wrap { position: fixed; bottom: 0; left: 0; width: 100%; overflow: hidden; height: 55px; background-color: #002244; box-sizing: border-box; z-index: 99999; border-top: 3px solid #ff8800; display: flex; align-items: center; box-shadow: 0px -5px 15px rgba(0,0,0,0.3); }
...
icone_mudo = '''<div style="position: fixed; bottom: 75px; left: 20px; z-index: 9999; opacity: 0.25;" title="Áudio em Espera"><svg width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="#666666" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="1" x2="1" y2="23"></line></svg></div>'''
icone_ativo = '''<div style="position: fixed; bottom: 75px; left: 20px; z-index: 9999; opacity: 0.8;" title="Áudio Ativo"><svg width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="#2e7d32" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"></path></svg></div>'''
...
function tocarAlertaChamaAtencao() {
    try {
        ...
        function tocarSino(frequencia, inicio, duracao) {
            ...
            osc.type = 'triangle'; osc.frequency.setValueAtTime(frequencia, inicio);
            gain.gain.setValueAtTime(0, inicio); gain.gain.linearRampToValueAtTime(3.0, inicio + 0.05); gain.gain.exponentialRampToValueAtTime(0.01, inicio + duracao);
            osc.connect(gain); gain.connect(ctx.destination); osc.start(inicio); osc.stop(inicio + duracao + 0.1);
        }
...
with CONTEUDO_TV.container():
    ...
    if st.session_state.idx == 4:
        ...
        # Sincronização invisível de 5 em 5 minutos
    ...
    # -------------------------------------------------------------------------
        ...
        if os.path.exists(ARQUIVO_ROTA_DISCO):
            ...
            if col_status_real:
                ...
                # LAÇO EM 2 COLUNAS PARA DEIXAR MAIOR E RESPONSIVO
            ...
            if col_gpon and len(cols_os) > 0 and col_status:
                ...
                if not df_gpon.empty:
                    ...
                    if not df_mig.empty:
                        ...
                        # 2 COLUNAS DE ACORDO COM O PEDIDO
                        for i in range(0, len(SUPS_ABC), 2):
                                ...
                                <div class="sup-card">
                                    <div class="sup-header">
                                        ...
                                        <div style="display: flex; gap: 8px; align-items: center;">
                                            <div style="background: #f8f9fa; color: #333; border: 2px solid #ccc; padding: 4px 10px; border-radius: 8px; font-size: 16px; font-weight: bold;">O.S.: {int(total_tarefas)}</div>
                                            <div style="background: #ffebee; color: {cor_q}; border: 2px solid {cor_q}; padding: 4px 10px; border-radius: 8px; font-size: 16px; font-weight: bold;">Quebra: {quebra:.1f}%</div>
                                            <div style="background: #e3f2fd; color: #006064; border: 2px solid #006064; padding: 4px 10px; border-radius: 8px; font-size: 16px; font-weight: bold;">Média: {media_equipe:.2f}</div>
                                        </div>
                                    </div>
        ...
        if os.path.exists(ARQUIVO_ROTA_DISCO):
            ...
            if col_status:
                ...
                <div class="box-base" style="padding: 10px 10px; margin-bottom: 15px; border-left: 10px solid #003366; background: #e3f2fd;">
                    <div style="display: flex; justify-content: space-around; align-items: center; background: #ffffff; padding: 10px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 8px;">
                        <div style="text-align: center;">
                            <div style="font-size: 16px; font-weight: bold; color: #666;">TOTAL DE O.S.</div>
                            <div style="font-size: 45px; font-weight: 900; color: #003366; line-height: 1;">{int(total_tarefas_op)}</div>
                        </div>
