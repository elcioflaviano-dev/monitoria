...
# =========================================================================
# 🧠 CORREÇÃO CRÍTICA: LÊ O EXCEL APENAS 1 VEZ A CADA 5 MINUTOS
# =========================================================================
@st.cache_data(ttl=300)
def carregar_supervisores_online():
    try:
        ...
        if not df_equipe.empty and len(df_equipe.columns) >= 3:
            ...
            return [str(s).strip().upper() for s in df_equipe["SUPERVISOR"].dropna().unique().tolist() if str(s).strip() != ""]
    except Exception:
        pass
    return []

SUPERVISORES = carregar_supervisores_online()
planilha_carregada = len(SUPERVISORES) > 0
