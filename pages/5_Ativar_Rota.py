...
# 1. Configuração Inicial (Menu Lateral vai aparecer agora)
st.set_page_config(layout="wide")

# 2. Inicialização SEGURA do Session State (DEVE VIR ANTES DE USAR)
...
# CSS PARA LIMPEZA (Menu lateral reativado via comentário)
st.markdown("""
    <style>
    [data-testid="stHeader"] { visibility: hidden !important; }
    .stDeployButton { display: none !important; }
    footer { visibility: hidden !important; }
    /* Para esconder o menu novamente, descomente a linha abaixo: */
...
# Lógica de exibição com verificação
if 'df_rota_ativa' in st.session_state and st.session_state['df_rota_ativa'] is not None:
    ...
    st.error("⚠️ Nenhum dado carregado na sessão. Por favor, certifique-se de que a página de processamento carregou o arquivo primeiro.")
