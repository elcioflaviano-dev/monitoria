...
if os.path.exists(ARQUIVO_ROTA_DISCO):
    try:
        ...
        # Procura EXATAMENTE pela coluna "Tipo de Atividade3" que você vai adicionar
        ...
        if col_status:
            ...
            if col_tipo_exata:
                # SE A COLUNA EXISTIR: Faz o filtro estrito e perfeito
                mask_base = df[col_tipo_exata].fillna('').astype(str).str.strip().str.lower() == 'na base'
            else:
                # FALLBACK DE SEGURANÇA: Enquanto você não adiciona a coluna, ele continua a procurar em todas as outras
            ...
            if len(nomes_na_base) == 0:
                if col_tipo_exata:
                    ...
                    st.warning("⚠️ A coluna 'Tipo de Atividade3' ainda não foi detectada no CSV. Tentando usar outras colunas de Tipo.")
