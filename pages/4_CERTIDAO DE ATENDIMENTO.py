...
# BUSCA A BASE CONSOLIDADA DA PÁGINA INICIAL
...
if df_master is not None and not df_master.empty:
    ...
    lista_janela = [str(x).strip() for x in pd.DataFrame(df_master[col_janela]).iloc[:, 0].fillna('Padrão').tolist()] if col_janela in df_master.columns else ['Padrão'] * len(df_master)
    ...
    lista_status_os1 = [str(x).strip() for x in pd.DataFrame(df_master['STATUS_OS1']).iloc[:, 0].fillna('NÃO EXECUTADO').tolist()] if 'STATUS_OS1' in df_master.columns else ['NÃO EXECUTADO'] * len(df_master)
    lista_recurso = [str(x).strip() for x in pd.DataFrame(df_master['Recurso']).iloc[:, 0].fillna('Não Identificado').tolist()] if 'Recurso' in df_master.columns else ['Não Identificado'] * len(df_master)
    ...
    df_base_online = pd.DataFrame({
        'Contrato': lista_contrato, 'Intervalo de Tempo': lista_janela, 'Status da Atividade': lista_status_at,
        'Status da O.S 1': lista_status_os1, 'Recurso': lista_recurso, 'SUPERVISOR': lista_supervisor
    })

# === AUTOMATIZAÇÃO DAS JANELAS ===
...
if df_base_online is not None:
    st.markdown(f'<div style="text-align: center; color: #cc6600; font-size: 14px; font-weight: bold; margin-bottom: 20px;">⏰ Painel Sincronizado com os Múltiplos Arquivos da Home</div>', unsafe_allow_html=True)
else:
    st.markdown('<div style="text-align: center; color: #cc6600; font-size: 13px; font-weight: bold; margin-bottom: 20px;">⚠️ Aguardando upload dos arquivos de rota na página inicial</div>', unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with col1:
        ...
        contrato_input = st.text_input("Número do Contrato:", value=valor_padrao_input, placeholder="Digite...").strip()
        st.session_state["limpar_input_proxima"] = False
    ...
    if contrato_input and df_base_online is not None:
        ...
        if not contrato_encontrado.empty:
            ...
            detalhes_validacao = "❌ Contrato não localizado nas rotas subidas hoje."

    if contrato_input:
        st.caption(detalhes_validacao)

    with col2:
        status_final = st.selectbox("Resultado:", ["OK", "NOK", "NÃO ADERENTE"])
    with col3:
        obs_input = st.text_input("Observações:", placeholder="Motivo...").strip()
    ...
    if st.button("💾 Gravar e Certificar Contrato", type="primary", use_container_width=True):
        if contrato_input != "":
            ...
            st.success(f"✅ Contrato {contrato_input} salvo localmente com sucesso!")
            st.rerun()
...
if df_base_online is not None:
                    ...
                    for _, row_p in df_cards_sup.iterrows():
                        ...
                        st.markdown(f"""
                            <div style="display:flex; justify-content:space-between; align-items:center; background-color:#f9f9f9; padding:6px 10px; border:1px solid #e0e0e0; border-radius:4px; margin-bottom:4px;">
                                <div style="font-size:13px; color:#333; font-weight:700;"><b>{c_num}</b> - 👤 {str(row_p['Recurso'])[:12].upper()}</div>
    ...
    st.info("ℹ️ Aguardando upload das rotas na página inicial.")
